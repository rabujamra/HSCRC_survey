...
# List of all Maryland hospitals (from the form - all 41!)
HOSPITALS = [
...
BEST_PRACTICES = {
    1: "Best Practice 1: Interdisciplinary Rounds & Early Discharge Planning",
    2: "Best Practice 2: Bed Capacity Alert System",
    3: "Best Practice 3: Standardized Daily Shift Huddles",
    4: "Best Practice 4: Expedited Care Intervention (Expediting team, expedited care unit)",
    5: "Best Practice 5: Patient Flow Throughput Performance Council",
    6: "Best Practice 6: Clinical Pathways & Observation Management"
}
...
*Please note: Each measure should be achieved in the tier you self-report on.*
""")
st.markdown('</div>', unsafe_allow_html=True)

# First Best Practice Selection
...
first_bp = st.selectbox(
    ...
    options=[""] + list(BEST_PRACTICES.values()),
    help="Required field"
)

# Show tier selection for first BP if selected
first_bp_tier = None
first_bp_notes = ""
...
if first_bp:
    ...
    first_bp_tier = st.radio(
        "Select the highest tier you plan to report on in the October 1st data collection: *",
        options=TIERS,
        key="first_tier",
        help="Required field"
    )
    ...
    first_bp_notes = st.text_area(
        "Optional: Are there any success stories and/or barriers to implementing this best practice you would like to share?",
        height=100,
        key="first_notes"
    )
    st.markdown('</div>', unsafe_allow_html=True)
...
# Filter out the first BP from second selection
available_for_second = [""] + [bp for bp in BEST_PRACTICES.values() if bp != first_bp]
...
second_bp = st.selectbox(
    ...
    help="Required field - Must be different from first selection"
)

# Show tier selection for second BP if selected
second_bp_tier = None
second_bp_notes = ""
...
if submit_button:
    ...
    if first_bp and second_bp and first_bp == second_bp:
        errors.append("❌ First and second best practices must be different")
    ...
    if errors:
        ...
        # Build data row - CLEAN 10-COLUMN FORMAT! 🎯
        timestamp = datetime.now().strftime("%m/%d/%Y %H:%M:%S")
        
        # Create data row with clean 10-column format
        data_row = {
            "Timestamp": timestamp,
            ...
            "BP1_Notes": first_bp_notes if first_bp_notes else "",
            "BP2_Name": second_bp,
            "BP2_Tier": second_bp_tier,
            "BP2_Notes": second_bp_notes if second_bp_notes else ""
        }
        
        # Save to CSV with proper quoting for fields containing commas
        df_new = pd.DataFrame([data_row])
        
        try:
            # Check if file exists
            if os.path.exists(DATA_FILE):
                # Append to existing file with proper quoting (handles commas in BP names!)
                df_new.to_csv(DATA_FILE, mode='a', header=False, index=False, quoting=1)
            else:
                # Create new file with header and proper quoting
                df_new.to_csv(DATA_FILE, mode='w', header=True, index=False, quoting=1)
            ...
            You can now view your submission in the **Hospital Portal** (if you have login credentials).
            
            If you need to make changes, please contact HSCRC staff or resubmit the form.
            """)
            st.markdown('</div>', unsafe_allow_html=True)
            ...
            st.download_button(
                ...
                file_name=f"{hospital_name.replace(' ', '_')}_submission_{datetime.now().strftime('%Y%m%d')}.csv",
                mime="text/csv"
            )
            
        except Exception as e:
            st.error(f"❌ **Error saving submission:** {str(e)}")
            st.markdown("Please contact HSCRC technical support.")
...
st.markdown("*For technical support, please contact HSCRC staff.*")
